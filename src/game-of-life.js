...
var GOL = {
  ...
  registerEvents: function() {
    ...
    this.helpers.registerEvent(
      document.getElementById("buttonCommit"),
      "click",
      this.handlers.buttons.commit,
      false
    );
    this.helpers.registerEvent(
      document.getElementById("buttonRestart"),
      "click",
      this.handlers.buttons.restart,
      false
    );
  },
    ...
    this.server.on("game", function(game) {
      ...
      for (c in GOL.players) {
        ...
        for (var i = 0; i < 4; i++) {
          ...
          GOL.automata.addCell(
            basePosition.x,
            basePosition.y,
            GOL.automata.actualState
          );
        }
      }
  ...
  handlers: {
    ...
    canvasMouseMove: function(event) {
      if (GOL.handlers.mouseDown) {
        ...
        var position = GOL.helpers.coordinatePosition(coordinates);
        if (
          position.x !== GOL.handlers.lastX ||
          position.y !== GOL.handlers.lastY
        ) {
          var r =
            (GOL.zoom.schemes[GOL.zoom.current].cellSize + 1) *
            GOL.territorySize;
          var click = [coordinates.x, coordinates.y];
          var base = [
            GOL.player.baseCoordinates.x,
            GOL.player.baseCoordinates.y
          ];
          if (GOL.helpers.distance(click, base) < r) {
            GOL.canvas.queueCell(position.x, position.y);
          }
    ...
    drawWorld: function() {
      ...
      if (GOL.gameOver) {
        ...
        this.context.fillText(
          "Game Over",
          (this.cellSize + this.cellSpace) * GOL.messageLoc[0],
          (this.cellSize + this.cellSpace) * GOL.messageLoc[1]
        );
        ...
        this.context.fillText(
          GOL.gameResult,
          (this.cellSize + this.cellSpace) * GOL.messageLoc[0],
          (this.cellSize + this.cellSpace) * GOL.messageLoc[1] + 30
        );
      }
      // Draw Territory
  ...
  automata: {
    ...
    nextGeneration: function() {
      ...
      var reference = _.flatten(
        this.actualState.map(function(x) {
          var coordinateList = [];
        ...
        true
      ); //true to only flatten one level

      for (p in GOL.players) {
        if (
          GOL.helpers.objectIntersection(reference, GOL.players[p].base)
            .length < 4
        ) {
          GOL.gameOver = true;
